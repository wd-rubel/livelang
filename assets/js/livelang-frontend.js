(function () {
  ...
  class LiveLangEditor {
    ...
    buildBar() {
      ...
      bar.id = "livelang-toggle";
      bar.className = "livelang-bar";
      bar.contentEditable = "false";

      const languages = this.getAllLanguages();
      const currentLang = this.settings.currentLanguage;
      const currentLabel = languages[currentLang] || 'Language';

      // Build language list items
      const langItems = Object.entries(languages)
        .map(([code, name]) => {
           const activeClass = code === currentLang ? ' class="active"' : '';
           return `<li><a href="#" data-lang="${code}"${activeClass}>${name}</a></li>`;
        })
        .join("");

      // inner html: actions + main button
      bar.innerHTML =
        '<div class="livelang-bar-actions">' +
        '<button type="button" id="livelang-save-btn" data-action="save">💾 ' +
        this.settings.i18n.save +
        "</button>" +
        '<label class="livelang-global-label">' +
        '<input type="checkbox" class="livelang-global"> ' +
        this.settings.i18n.global +
        "</label>" +
        // Custom Dropdown
        '<div class="livelang-language-dropdown">' +
        '<button class="livelang-language-toggle" type="button">' + 
        currentLabel + ' <span class="livelang-toggle-icon">▼</span>' + 
        '</button>' +
        ...
        // End Custom Dropdown
        "</div>" +
        '<button type="button" class="livelang-bar-main">Translate</button>';

      document.body.appendChild(bar);

      this.bar = bar;
      this.actionsEl = bar.querySelector(".livelang-bar-actions");
      this.mainButton = bar.querySelector(".livelang-bar-main");
      this.globalCheckbox = bar.querySelector(".livelang-global");
      ...
      // delegate actions (save / undo / redo)
    ...
    disableElementEvents(el) {
      ...
      el._livelang_original_pointer_events = el.style.pointerEvents || "";
      // Disable all pointer events (mouse, touch, etc.)
    ...
    restoreElementEvents(el) {
      ...
      if (el._livelang_original_pointer_events) {
        el.style.pointerEvents = el._livelang_original_pointer_events;
      } else {
    ...
    makeEditable(el) {
      ...
      // place caret at end for anchors and buttons - only on first edit
      if (el.tagName === "A" || el.tagName === "BUTTON") {
        if (!el._livelang_previously_edited) {
          this.placeCaretAtEnd(el);
          el._livelang_previously_edited = true;
        }
      }
    ...
    saveCurrent() {
      const saveButton = this.actionsEl.querySelector("#livelang-save-btn");
      const saveTexts = this.settings.i18n;
      saveButton.textContent = "💾 " + saveTexts.saving;
      ...
      data.append("action", "livelang_save_translation");
      data.append("_ajax_nonce", this.settings.nonce);
      data.append("original", originalText);
      data.append("translated", translatedText);
      data.append("slug", this.settings.slug);
      data.append("language", this.settings.currentLanguage);
      data.append(
        "is_global",
        this.globalCheckbox && this.globalCheckbox.checked ? "1" : "0",
      );

      //return // return for debugging

      xhr.open("POST", this.settings.ajaxUrl, true);
      xhr.onload = function () {
        // MVP: silent success/fail
        saveButton.textContent = "💾 " + saveTexts.saved;
        setTimeout(() => {
          saveButton.textContent = "💾 " + saveTexts.save;
        }, 2000);
      };
      xhr.send(data);
